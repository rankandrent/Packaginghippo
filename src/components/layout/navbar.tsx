        ...
        <header className="site-navbar fixed w-full z-50 bg-black/95 backdrop-blur-sm border-b border-gray-800">
            <div className="container mx-auto px-4 h-20 flex items-center justify-between">
                ...
                <Link href="/" className="flex items-center gap-2">
                    <div className="text-2xl font-black text-yellow-500 tracking-tighter uppercase mb-0">
                        {siteName.replace("Hippo", "")}<span className="text-white">Hippo</span>
                    </div>
                </Link>
            ...
            {isOpen && (
                <div className="md:hidden bg-black border-t border-gray-800 p-4 space-y-4">
                    <Link href="/products" className="block text-white font-medium" onClick={() => setIsOpen(false)}>Products</Link>
                    <Link href="/services" className="block text-white font-medium" onClick={() => setIsOpen(false)}>Services</Link>
                    <Link href="/quote" className="block bg-yellow-500 text-black text-center py-3 rounded-lg font-bold" onClick={() => setIsOpen(false)}>
                        Get Quote
                    </Link>
                </div>
            )}
