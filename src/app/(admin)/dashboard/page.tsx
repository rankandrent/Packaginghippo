...
export default function DashboardPage() {
    ...
    async function fetchStats() {
        try {
            ...
            if (pagesRes.ok) try { statsData.pages = await pagesRes.json() } catch (e) { console.error("Error parsing pages:", e) }
            if (productsRes.ok) try { statsData.products = await productsRes.json() } catch (e) { console.error("Error parsing products:", e) }
            if (categoriesRes.ok) try { statsData.categories = await categoriesRes.json() } catch (e) { console.error("Error parsing categories:", e) }
            if (sectionsRes.ok) try { statsData.sections = await sectionsRes.json() } catch (e) { console.error("Error parsing sections:", e) }

            setStats({
                pages: statsData.pages.pages?.length || 0,
                products: statsData.products.products?.length || 0,
                categories: statsData.categories.categories?.length || 0,
                sections: statsData.sections.sections?.length || 0,
            })
        } catch (error) {
