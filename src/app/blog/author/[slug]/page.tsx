                    ...
                    <div className="max-w-4xl mx-auto flex flex-col md:flex-row gap-12 items-center md:items-start text-center md:text-left">
                        ...
                        <div className="w-40 h-40 rounded-full overflow-hidden bg-white border-4 border-white shadow-xl shrink-0">
                            {author.image ? (
                                ...
                                <div className="w-full h-full flex items-center justify-center bg-blue-100 text-[#011f7b] font-black text-5xl uppercase">
                        ...
                        <div className="flex-1 space-y-6">
                            <div>
                                <h1 className="text-4xl md:text-5xl font-black text-[#011f7b] uppercase tracking-tight mb-2">{author.name}</h1>
                                <p className="text-xl font-bold text-[#01154f] uppercase tracking-widest">{author.role || 'Packaging Expert'}</p>
                            </div>
                            ...
                            <div className="flex flex-wrap justify-center md:justify-start gap-6">
                                <div className="flex items-center gap-2 text-sm font-bold text-gray-500 uppercase tracking-wider">
                                    <MapPin className="w-4 h-4 text-[#011f7b]" /> Global Expert
                                </div>
                                <div className="flex items-center gap-2 text-sm font-bold text-gray-500 uppercase tracking-wider">
                                    <GraduationCap className="w-4 h-4 text-[#011f7b]" /> {author.posts.length} Articles Published
                                </div>
                            </div>
                            ...
                            {author.socialLinks && (author.socialLinks as any).twitter || (author.socialLinks as any).linkedin || (author.socialLinks as any).website ? (
                                <div className="flex justify-center md:justify-start gap-4 pt-4 border-t w-fit mx-auto md:mx-0">
                                    {(author.socialLinks as any).linkedin && (
                                        <Link href={(author.socialLinks as any).linkedin} target="_blank" className="text-gray-400 hover:text-[#011f7b] transition-colors">
                                    ...
                                    {(author.socialLinks as any).website && (
                                        <Link href={(author.socialLinks as any).website} target="_blank" className="text-gray-400 hover:text-[#011f7b] transition-colors">
                                            <ExternalLink className="w-5 h-5" />
                                        </Link>
                                    )}
                                </div>
            ...
            <div className="container mx-auto px-4">
                <div className="max-w-6xl mx-auto">
                    <div className="flex items-center justify-between mb-12 border-b pb-6">
                        <h2 className="text-3xl font-black text-[#011f7b] uppercase tracking-tight">Expert Articles By {author.name}</h2>
                    </div>
                            ...
                            <Link key={post.id} href={`/blog/${post.slug}`} className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all border border-gray-100 flex flex-col">
                                ...
                                <div className="p-6 flex-1 flex flex-col">
                                    <div className="flex items-center gap-4 text-xs text-gray-400 mb-4 font-bold uppercase tracking-wider">
                                        ...
                                        {post.category && <span className="text-[#011f7b] font-black">{post.category.name}</span>}
                                    </div>
                                    <h3 className="text-xl font-black text-[#011f7b] mb-4 group-hover:text-[#01154f] transition-colors line-clamp-2 uppercase leading-snug">
                                        {post.title}
                                    </h3>
                                    ...
                                    <div className="flex items-center text-[#011f7b] font-black uppercase tracking-wider text-xs">
                    ...
                    {author.posts.length === 0 && (
                        <div className="text-center py-20 bg-gray-50 rounded-3xl border border-dashed text-gray-400">
                            ...
                            <Link href="/blog" className="text-[#011f7b] underline mt-4 inline-block font-bold uppercase">View all insights</Link>
                        </div>
                    )}
                    ...
                    <div className="mt-20 text-center">
                        <Link
                            href="/blog"
                            className="inline-flex items-center gap-2 text-sm font-black text-[#011f7b] uppercase hover:text-[#01154f] transition-colors border-2 border-[#011f7b] px-8 py-3 rounded-xl"
                        >
